...
//	
//	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_4;		//FLASH片选初始化
...
//	//FLASH_TYPE=AT45DB081B_ReadID();	//读取FLASH ID
//}
...
//{  
//	u8 byte=0;   
//	AT45DB081B_CS=0;                            //使能器件   
//	SPI1_ReadWriteByte(W25X_ReadStatusReg);    //发送读取状态寄存器命令    
//	byte=SPI1_ReadWriteByte(0Xff);             //读取一个字节  
//	AT45DB081B_CS=1;                            //取消片选     
//	return byte;   
//} 
////写AT45DB081B状态寄存器
...
//	while(1) 
//	{	
//		AT45DB081B_Read(AT45DB081B_BUF,secpos*4096,4096);//读出整个扇区的内容
//		for(i=0;i<secremain;i++)//校验数据
//		{
//			if(AT45DB081B_BUF[secoff+i]!=0XFF)break;//需要擦除  	  
//		}
//		if(i<secremain)//需要擦除
//		{
//			AT45DB081B_Erase_Sector(secpos);//擦除这个扇区
//			for(i=0;i<secremain;i++)	   //复制
//			{
//				AT45DB081B_BUF[i+secoff]=pBuffer[i];	  
//			}
//			AT45DB081B_Write_NoCheck(AT45DB081B_BUF,secpos*4096,4096);//写入整个扇区  

//		}else AT45DB081B_Write_NoCheck(pBuffer,WriteAddr,secremain);//写已经擦除了的,直接写入扇区剩余区间. 				   
...
//	AT45DB081B_CS=1;                            //取消片选     	      
//    AT45DB081B_Wait_Busy();   				   //等待擦除完成
//}  
////等待空闲
//void AT45DB081B_Wait_Busy(void)   
//{   
//	while((AT45DB081B_ReadSR()&0x01)==0x01);   // 等待BUSY位清空
//}  
////进入掉电模式
